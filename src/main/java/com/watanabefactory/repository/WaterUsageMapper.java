...
public interface WaterUsageMapper {
	...
	@Select("select water_amount, updated_at from water_usage where user_id = #{userId}")
	List<WaterDailyUsage> findByUserId(Integer userId);
	
	@Insert("insert into water_usage (user_id, water_amount, created_at, updated_at, deleted_flag) values (#{userId}, #{waterAmount}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0);")
